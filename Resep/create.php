    ...
    <head>
        ...
        if(isset($_GET['do']) && $_GET['do'] == 'tulis') {
            if(isset($_POST['judul']) && $_POST['judul'] != "") {
                if(isset($_POST['bahan']) && $_POST['bahan'] != "") {
                    if(isset($_POST['cara']) && $_POST['cara'] != "") {
                        if(isset($_POST['jenis']) && $_POST['jenis'] != "") {
                            if(isset($_FILES['foto']['name']) && !empty($_FILES['foto']['name'])){
                                ...
                                if($format_foto == 'image/jpeg' || $format_foto == 'image/jpg' || $format_foto == 'image/png' || $format_foto == 'image/gif') {
                                    ...
                                    $location = '../upload/resep/';
                                    if(move_uploaded_file($temp_name, $location.$_FILES['foto']['name'])){
                                        $foto       = $_FILES['foto']['name'];
                                        $query_res = 'INSERT INTO tb_temp_resep(id_akun, temp_judul, temp_bahan, temp_cara, temp_foto, temp_jenis) VALUES("'.$_SESSION['login'].'","'.$judul.'","'.$bahan.'","'.$cara.'","'.$foto.'","'.$jenis.'")';
                                        $result = mysqli_query($connect, $query_res);
                                        
                                        if($result) {
                                            ?> <script> alert("Resep berhasil dikirim"); </script> <?php
                                        } else { ?> <script> alert("Resep gagal dikirim"); </script> <?php }
                                    }
                                } else { ?> <script> alert("Format file tidak diperbolehkan"); </script> <?php }
                            } else { ?> <script> alert("Foto tidak boleh kosong"); </script> <?php }
                        } else { ?> <script> alert("Jenis tidak boleh kosong"); </script> <?php }
                    } else { ?> <script> alert("Cara tidak boleh kosong"); </script> <?php }
                } else { ?> <script> alert("Bahan tidak boleh kosong"); </script> <?php }
    ...
    <body>
        ...
        <div class="container">
            <section class="art-cont">
                <h1 style="font-size: 2em;">Tulis Resep</h1>
                *Resep tidak akan muncul sebelum dikonfirmasi oleh admin<hr>
                <form action="./create.php?do=tulis" method="post" enctype="multipart/form-data">
                    <div class="img-parent art-pict-up">
                        <img class="img" id="img" src="../images/food.jpg">
                    </div>
                    <input type="file" name="foto" id="foto" onchange="readURL(this)" required>
                    <select name="jenis" id="jenis" style="float: right; margin-top: 20px;">
                        <option value="makan">Makanan</option>
                        <option value="minum">Minuman</option>
                        <option value="snack">Snack</option>
                    </select>
                    <span style="float: right; margin-top: 20px;">Jenis:&emsp;</span>
                    <div class="akun-desc">
                        <input type="text" name="judul" id="nama" placeholder="Judul" required><br>
                        <textarea name="bahan" id="bahan" placeholder="Bahan" required></textarea><br>
                        <textarea name="cara" id="cara" placeholder="Cara" required></textarea><br>
                        <span style="float:left">*Tekan enter untuk menambah item</span>
