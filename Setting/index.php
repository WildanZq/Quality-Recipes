    ...
    <head>
        ...
        if(isset($_GET['do']) && $_GET['do'] == 'edit') {
            if(isset($_POST['nama']) && $_POST['nama'] != "") {
                ...
                if($foto_valid) {
                    $query = 'UPDATE tb_akun SET nama = "'.$nama.'", tentang = "'.$tentang.'", foto = "'.$foto.'" WHERE id_akun = '.$_SESSION['login'];
                    $result = mysqli_query($connect, $query);
                    
                    if($result) {
                        ?> <script> alert("Edit akun berhasil"); </script> <?php
                    } else { ?> <script> alert("Edit akun gagal"); </script> <?php }
                    
                } else { ?> <script> alert("Format file tidak diperbolehkan"); </script> <?php }
            } else { ?> <script> alert("Nama tidak boleh kosong"); </script> <?php }
        }
        ?>
    </head>
    <body>
                ...
                <form action="./?do=edit" method="post" enctype="multipart/form-data">
                    <div class="pp-cont">
                        <div class="img-parent akun-pp">
                            <img class="img" id="img" src="../<?php if($foto == "") { echo "images/pp.jpg"; } else { echo "upload/pp/".$foto; }?>">
                        </div>
                        <input type="file" name="foto" id="foto" onchange="readURL(this)">
                    </div>
                    <div class="akun-desc">
                        <input type="text" name="nama" id="nama" value="<?php echo $nama; ?>" required><hr>
                        <textarea name="tentang" id="tentang" placeholder="Jelaskan tentang anda"><?php echo $tentang; ?></textarea>
